...
const Dashboard = () => {
  ...
  const onSubmit: SubmitHandler<FormInputType>= async(data)=>{  
    try {
        ...
        const response = await api.post("/api/v1/video", formData, {headers:{"Content-Type" : "multipart/form-data"}})
      ...
      if (error.response?.data?.message) {
        ...
        setUploadError("An error occurred during uploading file. Please try again.");
      }
    } finally{
        setUploading(false)
    }
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-y-7">
                    ...
                    <div>
                        <div className="flex gap-x-4">
                            <label htmlFor="" className="min-w-20">Video</label>
                            <Input type="file" accept="video/*" placeholder="Description" {...register("videoFile", {required:"Vidoe file is required"})} />
                        </div>
                        {errors?.videoFile?.type==="required" && <p className="text-red-600">Video file is required</p>}
                    </div>
                    <div>
                        <div className="flex gap-x-4">
                            <label htmlFor="">Thumbnail</label>
                            <Input type="file" accept="image/*" placeholder="Description" {...register("thumbnail", {required:"Thumbnail is required"})} />
                        </div>
                        {errors?.videoFile?.type==="required" && <p className="text-red-600">Thumbnail File is required</p>}
                    </div>
                    <Input className="hover:bg-primary cursor-pointer font-semibold" type="submit" 
